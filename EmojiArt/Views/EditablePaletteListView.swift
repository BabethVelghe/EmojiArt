...
struct EditablePaletteListView: View {
    ...
    var body: some View {
        ...
        .navigationDestination(for: Palette.ID.self) { paletteId in
            if let index = store.palettes.firstIndex(where: { $0.id == paletteId }) {
                PaletteEditorView(palette: $store.palettes[index])
            }
        }
